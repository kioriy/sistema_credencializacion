...
Hoy el flujo de impresión usa una sola plantilla para toda la cola: el usuario elige una plantilla en el Panel de Control y el Centro de Impresión renderiza todos los registros con la plantilla del primer ítem de la cola. Con esta funcionalidad, el sistema evalúa un conjunto de reglas del tipo "atributo igual a valor" (por ejemplo, `grado == 2` o `nivel_escolar == "Primaria"`) y asigna a cada registro la plantilla destino indicada por la regla coincidente, con una plantilla por defecto cuando ninguna regla coincide.

La funcionalidad incluye: (1) un diálogo de configuración que lista las plantillas/diseños disponibles del cliente y permite asignar a cada uno una condición sobre un atributo del registro; (2) la persistencia de esa configuración en un lugar que el flujo de impresión pueda leer; (3) un botón de configuración (icono de engranaje) ubicado junto a la vista previa individual de frente/vuelta en el Editor de Plantillas; y (4) la evaluación automática de las reglas durante el armado de la cola y/o la generación del PDF.
...
- **Dialogo_Multiplantillaje**: Ventana modal que lista las plantillas/diseños disponibles del cliente y permite crear o editar la Configuracion_Multiplantillaje asignando condiciones por atributo.
- **Configuracion_Multiplantillaje**: Conjunto persistido que agrupa las reglas de asignación de plantilla base de un cliente, más la referencia a la Plantilla_Por_Defecto.
- **Regla_Asignacion**: Unidad de la Configuracion_Multiplantillaje compuesta por un atributo, un operador de igualdad y un valor, asociada a una Plantilla_Destino. Se expresa como "atributo igual a valor → Plantilla_Destino".
...
3. THE Dialogo_Multiplantillaje SHALL permitir al usuario seleccionar una o más plantillas de la lista para incluirlas en la Configuracion_Multiplantillaje.
4. IF el Cliente tiene una sola plantilla disponible, THEN THE Dialogo_Multiplantillaje SHALL mostrar un mensaje que indique que se requieren al menos dos plantillas para configurar el multiplantillaje.
5. IF la carga de la lista de plantillas del Cliente falla, THEN THE Dialogo_Multiplantillaje SHALL mostrar un mensaje de error que indique el fallo de la carga y SHALL mantener deshabilitada la acción de guardar la Configuracion_Multiplantillaje.
6. WHILE el usuario no ha seleccionado al menos una plantilla destino, THE Dialogo_Multiplantillaje SHALL mantener deshabilitada la acción de guardar la Configuracion_Multiplantillaje.

### Requisito 3: Definir reglas de asignación por atributo

**Historia de usuario:** Como diseñador de credenciales, quiero asignar a cada diseño seleccionado una condición sobre un atributo del registro, para que el sistema sepa qué plantilla usar según el valor del atributo.

#### Criterios de Aceptación

1. WHEN el usuario selecciona una plantilla destino en el Dialogo_Multiplantillaje, THE Dialogo_Multiplantillaje SHALL permitir definir una Regla_Asignacion compuesta por un Atributo, un operador de igualdad y un valor de hasta 255 caracteres.
2. THE Dialogo_Multiplantillaje SHALL ofrecer la selección del Atributo a partir de los Atributos_Disponibles del Cliente.
3. IF los Atributos_Disponibles del Cliente están vacíos, THEN THE Dialogo_Multiplantillaje SHALL impedir la creación de la Regla_Asignacion y mostrar un mensaje que indique que no existen atributos disponibles.
4. WHEN el usuario selecciona un Atributo para una Regla_Asignacion, THE Dialogo_Multiplantillaje SHALL permitir especificar el valor que dispara la asignación de la Plantilla_Destino, con un valor de longitud mínima de 1 carácter y máxima de 255 caracteres.
5. THE Dialogo_Multiplantillaje SHALL representar cada Regla_Asignacion como la correspondencia "Atributo igual a valor asigna Plantilla_Destino".
6. IF el usuario intenta guardar una Regla_Asignacion sin Atributo o sin valor, THEN THE Dialogo_Multiplantillaje SHALL rechazar el guardado, conservar los datos ya ingresados sin alterarlos y mostrar un mensaje que indique cada campo faltante.
7. IF el usuario intenta guardar una Regla_Asignacion cuyo par Atributo y valor coincide con el de una Regla_Asignacion ya existente en la Configuracion_Multiplantillaje, THEN THE Dialogo_Multiplantillaje SHALL rechazar el guardado y mostrar un mensaje que indique la regla duplicada.
8. THE Dialogo_Multiplantillaje SHALL permitir designar exactamente una de las plantillas como Plantilla_Por_Defecto de la Configuracion_Multiplantillaje.

### Requisito 4: Persistir la configuración de multiplantillaje

**Historia de usuario:** Como usuario del sistema, quiero que la configuración de multiplantillaje quede guardada por cliente, para que el flujo de impresión la utilice en cada impresión.

#### Criterios de Aceptación

1. WHEN el usuario confirma el guardado en el Dialogo_Multiplantillaje, THE Configuracion_Multiplantillaje SHALL persistirse asociada al único Cliente seleccionado, sustituyendo cualquier Configuracion_Multiplantillaje previa de ese Cliente.
2. THE Configuracion_Multiplantillaje SHALL almacenar la lista de Regla_Asignacion (de 0 a un máximo de 100 reglas) y una referencia obligatoria a la Plantilla_Por_Defecto.
3. WHEN la persistencia de la Configuracion_Multiplantillaje finaliza correctamente, THE Dialogo_Multiplantillaje SHALL mostrar una confirmación visible de guardado exitoso al usuario.
4. WHEN el Dialogo_Multiplantillaje se abre para un Cliente que ya tiene una Configuracion_Multiplantillaje, THE Dialogo_Multiplantillaje SHALL cargar y mostrar la lista completa de Regla_Asignacion y la Plantilla_Por_Defecto previamente guardadas.
5. THE Configuracion_Multiplantillaje SHALL ser legible por el Flujo_Impresion para resolver la plantilla de cada Registro.
6. IF la persistencia de la Configuracion_Multiplantillaje falla, THEN THE Dialogo_Multiplantillaje SHALL conservar sin modificaciones los cambios mostrados en pantalla y SHALL mostrar un mensaje de error que indique la causa del fallo.
7. IF la carga de una Configuracion_Multiplantillaje existente falla al abrir el Dialogo_Multiplantillaje, THEN THE Dialogo_Multiplantillaje SHALL mostrar un mensaje de error que indique la causa del fallo y SHALL abrirse sin reglas precargadas.
...
2. WHEN el valor del Atributo de un Registro coincide exactamente con el valor de una Regla_Asignacion (comparación sin distinción de mayúsculas/minúsculas y sin considerar espacios en blanco iniciales o finales), THE Motor_Asignacion SHALL asignar la Plantilla_Destino de esa regla al Registro.
3. IF ningún valor de Atributo del Registro coincide con alguna Regla_Asignacion, THEN THE Motor_Asignacion SHALL asignar la Plantilla_Por_Defecto al Registro.
4. WHEN los datos de un Registro coinciden con más de una Regla_Asignacion, THE Motor_Asignacion SHALL asignar la Plantilla_Destino de la primera regla coincidente según el orden definido en la Configuracion_Multiplantillaje.
5. WHEN el Flujo_Impresion arma la cola de impresión, THE Flujo_Impresion SHALL registrar en cada ItemCola la plantilla asignada por el Motor_Asignacion para el Registro correspondiente.
6. WHEN el Flujo_Impresion genera el PDF de una cola, THE Flujo_Impresion SHALL renderizar cada Registro con la imagen base y los elementos de frente y vuelta de la plantilla asignada a ese Registro.
7. WHERE no existe una Configuracion_Multiplantillaje para el Cliente, THE Flujo_Impresion SHALL conservar el comportamiento actual de usar la plantilla seleccionada para todos los registros de la cola.
8. IF ningún valor de Atributo del Registro coincide con alguna Regla_Asignacion y no existe una Plantilla_Por_Defecto configurada en la Configuracion_Multiplantillaje, THEN THE Motor_Asignacion SHALL dejar el Registro sin plantilla asignada y registrar una indicación de error que identifique el Registro afectado, conservando los datos del Registro sin modificación.
9. IF la plantilla asignada a un Registro no puede cargarse al generar el PDF (imagen base o elementos de frente o vuelta no disponibles), THEN THE Flujo_Impresion SHALL omitir la generación de ese Registro y presentar una indicación de error que identifique el Registro y la plantilla afectados, continuando con los Registros restantes de la cola.
...
1. WHEN el usuario edita una Regla_Asignacion existente en el Dialogo_Multiplantillaje, THE Dialogo_Multiplantillaje SHALL permitir cambiar el Atributo, el valor o la Plantilla_Destino de esa regla, conservando sin cambios los demás campos no editados de esa misma regla.
...
4. IF el usuario intenta confirmar los cambios cuando el valor de una Regla_Asignacion editada está vacío o cuando su Plantilla_Destino no referencia una plantilla existente, THEN THE Dialogo_Multiplantillaje SHALL rechazar la confirmación, mantener sin cambios la Configuracion_Multiplantillaje previamente persistida y mostrar un mensaje de error que indique el campo inválido.
5. WHEN el usuario confirma los cambios de edición y todas las Regla_Asignacion editadas tienen valor no vacío y Plantilla_Destino existente, THE Configuracion_Multiplantillaje SHALL persistir la versión actualizada asociada al Cliente dentro de 2 segundos.
...
1. IF un Registro no contiene el Atributo evaluado por una Regla_Asignacion, THEN THE Motor_Asignacion SHALL tratar esa Regla_Asignacion como no coincidente para ese Registro y continuar la evaluación con las Regla_Asignacion restantes sin interrumpir el procesamiento del Registro.
2. IF el valor del Atributo de un Registro no coincide con ninguna Regla_Asignacion, THEN THE Motor_Asignacion SHALL asignar la Plantilla_Por_Defecto al Registro.
...
8. WHEN el Motor_Asignacion evalúa el valor de un Atributo de un Registro contra el valor de una Regla_Asignacion, THE Motor_Asignacion SHALL comparar ambos valores como texto sin distinguir mayúsculas de minúsculas y eliminando los espacios al inicio y al final de cada valor antes de comparar.
